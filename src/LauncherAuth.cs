...
public sealed class LauncherAuth {
    ...
    public async Task<string> IssueGameTicket(string endpoint,CancellationToken token){var value=Token;if(String.IsNullOrEmpty(value))return null;using(var client=new HttpClient()){client.Timeout=TimeSpan.FromSeconds(10);client.DefaultRequestHeaders.TryAddWithoutValidation("Authorization","Bearer "+value);client.DefaultRequestHeaders.TryAddWithoutValidation("Origin","https://aioncl.com");var response=await client.PostAsync(endpoint,new StringContent("{}",Encoding.UTF8,"application/json"),token).ConfigureAwait(false);if(!response.IsSuccessStatusCode)return null;var data=Json.Parse<LauncherGameTicketResponse>(await response.Content.ReadAsStringAsync().ConfigureAwait(false));return data==null?null:data.ticket;}}
