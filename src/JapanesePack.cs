...
namespace AionCL {
public sealed class JapanesePack {
    readonly ClientFile[] files;
    public JapanesePack(ClientFile[] catalog) {
    ...
    internal async Task ChangeFiles(string root,string language,bool install,string source,IProgress<VerificationProgress> progress,CancellationToken token) {
        ...
        foreach(var f in files) {
            ...
            if(File.Exists(active) && !await Safety.Matches(active,f.size,f.sha256,token).ConfigureAwait(false)) throw new IOException("Modified Japanese pack file preserved: "+f.path);
